    ...
    for epoch in range(args.init_epochs,args.epochs):
        ...
        for i in range(args.niter):
            ...
            if (i+1) % args.n_critic == 0:
                ...
                recon_img_2d,recon_img_2d_mask = netG(img_o2d,s_label)
                recon_img_2d_mask = saturate_mask(recon_img_2d_mask, saturate=True)
                recon_imgs_2d_masked = recon_img_2d_mask * img_o2d + (1 - recon_img_2d_mask) * recon_img_2d

                fake_src_loss_2d = - torch.mean(out_src_2d)
                fake_cls_loss_2d = (mse_loss(out_cls_2d,t_label)/img.size(0))*args.lambda_cls*args.nserial
                fake_src_loss_3d = - torch.mean(out_src_3d)
                ...
                recon_loss = torch.mean(torch.abs(recon_imgs_2d_masked - img_o2d)*(1 - recon_img_2d_mask))*args.lambda_recon
...
def test(args, netG, device):
    ...
    with torch.no_grad():
        for index,(img,label, fname, n_data) in enumerate(test_loader):
            ...
            save_3dimg(args.test_out+"/"+fname[0][:-11]+"_NM.nii.gz",medi_format,(fake_imgs_00+1)*127.5)                    
            save_3dimg(args.test_out+"/"+fname[0][:-11]+"_MCI.nii.gz",medi_format,(fake_imgs_05+1)*127.5)                          
            save_3dimg(args.test_out+"/"+fname[0][:-11]+"_AD.nii.gz",medi_format,(fake_imgs_10+1)*127.5)                     
